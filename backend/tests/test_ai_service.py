...
class TestLogprobsRejection:
    ...
    NOTE (F.A.S.C. fork): upstream's fix adds `request_body["logprobs"]` and reads
    `choice.get("logprobs")` on the OpenAI-compatible response shape. Our
    ai_service.py talks to Ollama's native `/api/chat` (not `/v1/chat/completions`)
    for tagging calls, so it never sends or expects a logprobs param — see
    LOCAL-CHANGES.md. The two tests that asserted upstream's logprobs body/field
    were dropped as inapplicable; the retry-without-crashing-on-unrelated-400
    behavior below is still real and still covered.
    """

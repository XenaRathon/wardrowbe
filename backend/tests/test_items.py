...
class TestItemService:
    ...
    async def test_get_list_orders_ties_deterministically(
        ...
        # Our fork's get_list takes the User object (not a bare UUID) so it can
        # evaluate family-sharing scope; see app/services/item_service.py.
        first_ids = [item.id for item in (await service.get_list(test_user, filters))[0]]
        second_ids = [item.id for item in (await service.get_list(test_user, filters))[0]]
